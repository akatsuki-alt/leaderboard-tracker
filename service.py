...
def build_tasks(config: TrackerConfig) -> List[Task]:
    tasks = [TrackLiveLeaderboard(config), TrackClanLiveLeaderboard(config), ProcessQueue(config), RecalculateScores(config), TrackLinkedUserStats(config), TrackGenericLeaderboard('score', config), AutoqueueLinkedUsers(config), UpdateOldStats(config), TrackAkatLovedMapsScores(config)]
